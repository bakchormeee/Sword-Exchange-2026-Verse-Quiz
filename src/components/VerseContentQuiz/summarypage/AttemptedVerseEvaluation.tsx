...
const AttemptedVerseEvaluation = ({
  ...
  const accuracy: number = Math.max(
    0,
    Math.floor(
      100 -
        (levenshtein(
          verseSubmission.content.replace(/\p{P}/gu, "").toLowerCase(),
          verseSubmission.submittedContent.replace(/\p{P}/gu, "").toLowerCase(),
        ) /
          verseSubmission.content.replace(/\p{P}/gu, "").length) *
          100,
    ),
  );
